...
class Application(ctk.CTk):
    def __init__(self):
        ...
        self.frame = ctk.CTkFrame(self, fg_color=("#323232"), corner_radius=10, border_width=1, border_color="#0e0f16")
        ...
        self.send_button = ctk.CTkButton(self, text="Send", font=("Helvetica", 12))
    ...
    def create_message(self, message, sender="user"):
        ...
        message_widget = Message_Widget(self.frame, message, sender)
        self.message_frames.append(message_widget)
        
        if len(self.message_frames) > 5:
...
class Message_Widget(ctk.CTkFrame):
    def __init__(self, parent, message, sender):
        super().__init__(parent)
        self.message = message
        self.sender = sender
        self.message_label = ctk.CTkLabel(self, text=f"{sender}: {message}", font=("Helvetica", 12), wraplength=300, justify="left")
